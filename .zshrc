...
alias bub='current_branch=$(git rev-parse --abbrev-ref HEAD) && timestamp=$(date "+%Y.%m.%d-%k.%M") && backup_branch="${current_branch}_backup_${timestamp}" && git checkout -b "${backup_branch}" && git checkout "${current_branch}" && echo "Created branch ${backup_branch}"'
